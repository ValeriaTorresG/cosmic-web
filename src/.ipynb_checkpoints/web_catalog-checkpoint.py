...
# Mantén aquí tus funciones web_classification, find_friends, find_fof_groups,
# inertia_tensor y compute_group_properties sin cambios.
...
def main():
    init_t = t.time()
    p = argparse.ArgumentParser()
    p.add_argument('--base_dir',   default='/pscratch/sd/v/vtorresg/cosmic-web/data/dr1/entropy/')
    p.add_argument('--out_dir', default='/pscratch/sd/v/vtorresg/cosmic-web/data/dr1/catalog/')
    p.add_argument('--webtype',    default='all')
    p.add_argument('--void_limit', type=float, default=-0.9)
    p.add_argument('--knot_limit', type=float, default=0.5)
    args = p.parse_args()
    ...
    for region in REGIONS:
        for run in range(N_ITER):
            posfile   = os.path.join(args.base_dir, f'{region}_run{run}_pos.txt')
            pairfile  = os.path.join(args.base_dir, f'{region}_run{run}_pairs.txt')
            countfile = os.path.join(args.base_dir, f'{region}_run{run}_counts.txt')
            catalogf  = os.path.join(args.out_dir,  f'{region}_run{run}_{args.webtype}.csv')
            print(posfile, pairfile, countfile, catalogf)
            make_catalog(posfile, pairfile, countfile, catalogf,
                         args.webtype, args.void_limit, args.knot_limit)
            print(f'[Catalog] {region} run{run} -> {catalogf}')
