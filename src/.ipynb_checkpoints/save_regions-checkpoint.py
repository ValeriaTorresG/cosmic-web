...
#takes ~90s
#ngc1 1821322, ngc2 34432, sgc 74059
...
def process_run_region(run_idx, out_path, rand_files, cuts, n_reg):
    ...
    replace = len(sub_rand) < n_reg
    idx = np.random.choice(len(sub_rand), size=n_reg, replace=replace)
    subset = sub_rand[idx]
...
def write_data_randoms(base_dir, out_dir, n_runs, max_workers=8):
    ...
    for hemi in ['NGC','SGC']:
        ...
        for region in regs:
            ...
            with open(tmp,'rb') as fin,gzip.open(fn,'wb') as fout:shutil.copyfileobj(fin,fout)
            os.remove(tmp)
            print(f'  • Generando {n_runs} randoms')
            with ProcessPoolExecutor(max_workers=max_workers, initializer=seed_main) as exe:
                ...
                for fut in as_completed(futures):
                    i = futures[fut]
                    try: fut.result(); print(f'    · run {i} listo')
...
if __name__=='__main__':
    ...
    p.add_argument('--max_workers', type=int, default=15)
    args = p.parse_args()
